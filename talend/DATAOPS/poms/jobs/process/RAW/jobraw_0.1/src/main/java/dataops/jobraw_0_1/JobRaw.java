 ...
 * @status
 */
public class JobRaw implements TalendJob {
	...
	private final String jobName = "JobRaw";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(JobRaw.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:mPY/07pNkuGU5JWh88wMBd+cH7Zl7qBIO51OwP2H72KQbum3ow==");
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_DATAOPS_JobRaw = new byte[0];
		static byte[] commonByteArray_DATAOPS_JobRaw = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DATAOPS_JobRaw.length) {
					if (length < 1024 && commonByteArray_DATAOPS_JobRaw.length == 0) {
						commonByteArray_DATAOPS_JobRaw = new byte[1024];
					} else {
						commonByteArray_DATAOPS_JobRaw = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_DATAOPS_JobRaw, 0, length);
				strReturn = new String(commonByteArray_DATAOPS_JobRaw, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_DATAOPS_JobRaw, 0, length);
				strReturn = new String(commonByteArray_DATAOPS_JobRaw, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DATAOPS_JobRaw) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				fileName_tFileOutputDelimited_1 = (new java.io.File(
						"/Users/arthur/Arthur/FIAP/Trabalhos/DataOps/Talend/TOS_DI-macosx-cocoa.app/Contents/MacOS/workspace/DATAOPS/RAW/OrderSalesHeader.csv"))
	...
	public static void main(String[] args) {
		final JobRaw JobRawClass = new JobRaw();

		int exitCode = JobRawClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = JobRaw.class.getClassLoader()
					.getResourceAsStream("dataops/jobraw_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = JobRaw.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : JobRaw");
		}
...
/************************************************************************************************
 * 74710 characters generated by Talend Open Studio for Data Integration on the
 * 16 de fevereiro de 2022 22:18:12 BRT
 ************************************************************************************************/